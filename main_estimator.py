...
# tabout
# 
# tablout=rr1.pivot_table(values="volatility", index=['participant.code'], columns=['round'])
# tablout
...
theta_start = np.array([0.5, 0.05, r0])
params=theta_start
    ...
    def neg_loglike(params):
        ...
        y = (np.log(
            (((1 - np.exp(- r * tou['xup'])) / (1 - np.exp(-r * utmax))) * tou['probup']
                + lamb * ((1 - np.exp(-r * tou['xdown'])) / (1 - np.exp(-r * utmax))) * tou['probdown']) / tou['q']) - thet) / (np.sqrt(2) * nu)
